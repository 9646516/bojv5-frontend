    ...
    >
      <template v-slot:item="{ item }">
        <tr class="text-xs-center">
          ...
          <td v-for="(a,b) in problem" :key="a" class="font-weight-black">
            <span v-if="item[b]&&!item[b][1]" style="color:red;">-{{item[b][0]}}</span>
            <span v-else-if="item[b]&&item[b][1]" style="color:green;">+{{item[b][0]}}</span>
            <v-icon v-if="first[b]===item.Team" color="green">mdi-google</v-icon>
          </td>
        </tr>
      </template>
    </v-data-table>
      ...
      for (var i of mp) {
        ...
        if (i[0]) {
          if (!rank[i[2]].hasOwnProperty(i[3])) {
            ...
            rank[i[2]]["Solved"] += 1;
            rank[i[2]]["Penalty"] +=
              i[1].getTime() - this.openTime.getTime() + 20 * 60 * 1000;
            rank[i[2]]["Last"] = i[1].getTime();
            if (!this.first.hasOwnProperty(i[3])) {
              this.first[i[3]] = i[2];
            }
